...
export class NovoFuncionarioComponent {
  ...
  onFileChange(event: any, index: number): void {
    ...
    console.log(
      `Estado do documento ${index}:`,
      documentoControl.value
    );
  ...
  createFuncionario(): void {
    ...
    if (this.funcionarioForm.invalid) {
      ...
      console.log(
        'Formulário inválido:',
        this.funcionarioForm.errors
      );

      console.log(
        'Estado do FormArray documentos:',
        this.documentos.controls.map(c => c.value)
      );
    ...
    formData.append(
      'Nome',
      this.funcionarioForm.get('nome')?.value
    );

    formData.append(
      'Matricula',
      this.funcionarioForm.get('matricula')?.value
    );

    formData.append(
      'Telefone',
      this.funcionarioForm.get('telefone')?.value
    );

    formData.append(
      'Status',
      this.funcionarioForm.get('status')?.value
    );
    ...
    this.documentos.controls.forEach((doc, index) => {
      ...
      if (categoria && arquivo) {
        ...
        console.log(
          `Appending file: ${arquivo.name} with category: ${categoria} at index: ${index}`
        );
        ...
        console.warn(
          `Documento inválido no índice ${index}: categoria=${categoria}, arquivo=${arquivo ? arquivo.name : 'null'}`
        );
    ...
    if (!documentosValidos && this.documentos.length > 0) {
      ...
      console.error(
        'Um ou mais documentos estão incompletos.'
      );

      this.errorMessage =
        'Todos os documentos devem ter categoria e arquivo preenchidos.';
